...
//COLORING,TOUR,INTERACTIVE:
		...
		modeLabel.innerHTML=MODE_NAMES[0];
...
//LANGUAGES:
		...
		languageLabel.innerHTML=LANGUAGE_NAMES[LANGUAGE_NAMES["keys"][languageNum]];
...
//SAVE & RELOAD
		var saveStateB = document.createElement('input');
		saveStateB.setAttribute("type","button");
		saveStateB.className = 'save-load-state-button';
		saveStateB.setAttribute('id','save-state-button');
		saveStateB.setAttribute('value','Save State');
		saveStateB.onclick=function(){colormyworld.saveStateCB()};

		var loadStateB = document.createElement('input');
		loadStateB.setAttribute("type","button");
		loadStateB.className = 'save-load-state-button';
		loadStateB.setAttribute('id','load-state-button');
		loadStateB.setAttribute('value','Load State');
		loadStateB.onclick=function(){colormyworld.loadStateCB()};

		var saveloadTable=document.createElement("table");
		var r=saveloadTable.insertRow(-1);
		r.insertCell(-1).appendChild(saveStateB);
		r.insertCell(-1).appendChild(loadStateB);

		modeDiv.appendChild(modeTable);
		modeDiv.appendChild(regionTable);
		modeDiv.appendChild(languageTable);
		modeDiv.appendChild(saveloadTable);
