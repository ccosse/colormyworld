define(["activity/ol","util","print","activity/l10n",],function(ol,util,print,l10n){
	var me={};
	...
	me.DELAY=1500.;
	me.RUNNING=false;
	me.setTour=function(val){
		me.tour=val;
	}
	me.getTour=function(val){
		return me.tour;
	}
...
//		window.control_panel.rebuild();
	...
	me.prepare_layers=function(){
		...
		me.point_layers=[];
		for(var cidx=0;cidx<me.currents.length;cidx++){
		for(var pidx=0;pidx<INSTALLED[me.currents[cidx]]["point_sources"].length;pidx++){

			var src_url=INSTALLED[me.currents[cidx]]["path"] + INSTALLED[me.currents[cidx]]["point_sources"][pidx]["filename"];
			...
			var point_layer= new ol.layer.Vector({
				...
				style:new ol.style.Style({
					image:new ol.style.Circle({
						radius:INSTALLED[me.currents[cidx]]["point_sources"][pidx]["radius"],
						stroke: new ol.style.Stroke({
							color: INSTALLED[me.currents[cidx]]["point_sources"][pidx]["color"],
							width: INSTALLED[me.currents[cidx]]["point_sources"][pidx]["width"]
						}),
						fill: new ol.style.Fill({
							color: INSTALLED[me.currents[cidx]]["point_sources"][pidx]["fill"],
						}),
					})
				}),
			});
			...
			var category=INSTALLED[me.currents[cidx]]["point_sources"][pidx]["category"];
			layer_name=INSTALLED[me.currents[cidx]]["point_sources"][pidx]["layer_name"];
		...
		me.line_layers=[];
		for(var cidx=0;cidx<me.currents.length;cidx++){
		for(var lidx=0;lidx<INSTALLED[me.currents[cidx]]["line_sources"].length;lidx++){
			var src_url=INSTALLED[me.currents[cidx]]["path"] + INSTALLED[me.currents[cidx]]["line_sources"][lidx]["filename"];
			...
			var line_layer= new ol.layer.Vector({
				...
				style:new ol.style.Style({
					stroke: new ol.style.Stroke({
						color: INSTALLED[me.currents[cidx]]["line_sources"][lidx]["color"],
						width: INSTALLED[me.currents[cidx]]["line_sources"][lidx]["width"]
					}),
				})
			});
			line_layer.set("type","Line");

			var category=INSTALLED[me.currents[cidx]]["line_sources"][lidx]["category"];
			layer_name=INSTALLED[me.currents[cidx]]["line_sources"][lidx]["layer_name"];
		...
		me.gpx_layers=[];
		for(var cidx=0;cidx<me.currents.length;cidx++){
		for(var lidx=0;lidx<INSTALLED[me.currents[cidx]]["gpx_sources"].length;lidx++){
			var src_url=INSTALLED[me.currents[cidx]]["path"] + INSTALLED[me.currents[cidx]]["gpx_sources"][lidx]["filename"];
			...
			var category=INSTALLED[me.currents[cidx]]["gpx_sources"][lidx]["category"];
			layer_name=INSTALLED[me.currents[cidx]]["gpx_sources"][lidx]["layer_name"];
...
//GAME ORCHESTRATION:
	...
	me.startMove=function(feature){
		...
		if(document.webL10n.getLanguage()!='en-us'){
			var xhtml="<center><h3>"+util.descore(document.webL10n.get(util.enscore('Next')))+":</h3><h1>"+util.descore(document.webL10n.get(util.enscore(target_name)))+"</h1><h3>"+target_layer_name+"</h3></center>";
		}
