...
//Wipe-em all out here (keeping) ... rebuild (multiple) below ... check memory leak here
		...
		if(me.currents[0]=="World"){
			...
			window.map.getView().setCenter(ol.proj.transform(INSTALLED['World']['center'], 'EPSG:4326', 'EPSG:3857'));
...
//GAME ORCHESTRATION:
	...
	me.fastFill=function(){
		var count=0;
//		while(count<1){
//		try{
		var candidates=me.get_enabled_candidates();
		me.current_feature=candidates.shift();
		...
		print(me.CATEGORIES[category][layer_name]['features'].keys())

		//me.CATEGORIES[category][layer_name]['features'][feature_name]['feature'].setStyle(me.random_styles[ridx]);
		//me.current_feature.setStyle(me.random_styles[ridx]);
		count+=1;
		print("count = "+count);
...
//		document.getElementById("playB").innerHTML='<img src="./static/ggmc/img/flaticon/play.png" class="icon"/>';
	...
	me.random_styles=[];
	me.update_styles=function(){
		print("colormyworld.update_styles");
		me.random_styles=[];
		var colors=colorcfg.getColorsList();
			...
			for(var fidx=0;fidx<features.length;fidx++){
				...
				if(feature==target_feature){
					...
					if(me.mode==TOUR){
						...
						if(me.mode==INTERACTIVE){
							me.score+=1;console.log("me.score=",me.score);
							if(me.score>=me.target_score){
								try{
									console.log("calling showCollectCB ...");
