	...
	//		w3.includeHTML();//pity this didn't work in Chroome w/d3 ... w3 couldn't find container1
		...
		me.rgb_curves=function(prefix){
			...
			for(var xidx=0;xidx<255;xidx+=2){
				r_gen_data.push( [xidx,Math.max(0.1,100*Math.exp( -0.5*( (xidx-me.cfg[prefix]['red']['mu'])/(me.cfg[prefix]['red']['sigma']) *(xidx-me.cfg[prefix]['red']['mu'])/(me.cfg[prefix]['red']['sigma']) )) )] );
				g_gen_data.push( [xidx,Math.max(0.1,100*Math.exp( -0.5*( (xidx-me.cfg[prefix]['green']['mu'])/(me.cfg[prefix]['green']['sigma']) *(xidx-me.cfg[prefix]['green']['mu'])/(me.cfg[prefix]['green']['sigma']) )) )] );
				b_gen_data.push( [xidx,Math.max(0.1,100*Math.exp( -0.5*( (xidx-me.cfg[prefix]['blue']['mu'])/(me.cfg[prefix]['blue']['sigma']) *(xidx-me.cfg[prefix]['blue']['mu'])/(me.cfg[prefix]['blue']['sigma']) )) )] );
			}
